...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDIRECT){
    ...
    if(ip->type == T_CHECKED)
      // mask for retrieving addr, since we are using the top byte for checksum
      addr &= 0x00ffffff;
    return addr;
  }
  bn -= NDIRECT;
  ...
  for(i = 0; i < NDIRECT; i++){
    if(ip->addrs[i]){
      if(ip->type == T_CHECKED) {
        ...
        bfree(ip->dev, (ip->addrs[i] & 0x00ffffff));
      }
      ip->addrs[i] = 0;
    }
  }
...
stati(struct inode *ip, struct stat *st)
{
  ...
  struct buf *bp;
  uint* indirect_ip;
  uint checksum = 0, inode_sum = 0, tmp = 0;
  //initialize checksum
  if(ip->type == T_CHECKED){
    for(i = 0; i < (NDIRECT+1); i++){
      ...
      // Indirect pointer
        ...
        bp = bread(ip->dev, ip->addrs[NDIRECT]);
        // get all the pointers to indirect files, iterate and checksum
        indirect_ip = (uint*)bp->data;
        for(j = 0; j < (BSIZE/sizeof(uint)); j++) {
          inode_sum ^= (indirect_ip[j] >> 24);
        }
        checksum ^= inode_sum;
        // release buffer
        brelse(bp);
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
    ...
    // checksum if is corresponding file type
    if(ip->type == T_CHECKED){
      blknum = off/BSIZE;

      // direct ptr, retrieve and write checksum using bitmask
      if(blknum < NDIRECT) {
        inode_sum = (ip->addrs[blknum] & 0xff000000) >> 24;
      // indirect ptr
      } else {
        blknum -= NDIRECT;
        temp = bread(ip->dev, ip->addrs[NDIRECT]);
        indirect_ip = (uint*)temp->data;
        inode_sum = (indirect_ip[blknum] & 0xff000000) >> 24;
        brelse(temp);
      }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
    ...
    if(ip->type == T_CHECKED){
      ...
      for(i = 0; i < BSIZE; i++)
        cache_sum ^= bp->data[i];
      cache_sum = cache_sum << 24;

      // direct ptr
      if(blknum < NDIRECT) {
        ip->addrs[blknum] &= 0x00ffffff;
        ip->addrs[blknum] |= cache_sum;
      // indirect ptr
      } else {
        blknum -= NDIRECT;
        temp = bread(ip->dev, ip->addrs[NDIRECT]);
        indirect_ip = (uint*)temp->data;
        indirect_ip[blknum] &= 0x00ffffff;
        indirect_ip[blknum] |= cache_sum;
        bwrite(temp);
        brelse(temp);
      }
    }
